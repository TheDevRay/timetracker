        ...
        methods: {
            ...
            _readableTimeFromSeconds: function(seconds) {
                ...
                return {
                    ...
                    minutes: this._padNumber(seconds % 60),
                    seconds: this._padNumber(parseInt(seconds / 60, 10) % 60)
                };
             },
             /**
             ...
             */
             stopTimer: function() {
                 window.axios.post(`/projects/${this.counter.timer.id}/timers/stop`)
                    .then(response => {
                        // Loop through the projects and find the right one
                        this.projects.forEach(project => {
                            if (project.id === parseInt(this.counter.timer.id)) {
                                // Loop through the timers of the projects and set the `stopped_at` time
                                return this.timers.forEach(timer => {
                        ...
                        // Reset the counter and the timer string
                        this.counter = {
                            seconds: 0,
                            timer: null
                        }
